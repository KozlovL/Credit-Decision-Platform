...
def test_get_user_data_success(client, existing_user):
...
def test_put_create_new_user(client, new_user_payload):
...
def test_put_update_existing_profile(client, update_profile_payload):
...
def test_put_add_new_loan_entry(client, new_loan_entry_payload):
...
def test_put_update_existing_loan_status(client, update_loan_status_payload):
...
def test_put_combined_update(client, combined_update_payload):
...
def test_put_invalid_product(client, invalid_product_payload):
    """Несуществующее название продукта."""
    response = client.put(USER_DATA_URL, json=invalid_product_payload)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_put_invalid_date(client, invalid_date_payload):
    """Невалидная дата."""
    response = client.put(USER_DATA_URL, json=invalid_date_payload)
    assert response.status_code == HTTPStatus.UNPROCESSABLE_ENTITY


def test_put_duplicate_loan_id(client, duplicate_loan_id_payload):
